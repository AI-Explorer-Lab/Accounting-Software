...
`agent.harness_enabled=true` 时，每个 `agent.projects[]` 必须配置 `knowledge_actor_id`；当前 Accounting 项目为 `zhangsan`。全局 `agent.knowledge` 配置 Knowledge-Base、MCP registry 和归档 writer `orchestrator`。这些身份只用于知识访问与审计，不是登录账号。能力不可用时服务启动失败关闭，不会静默退回无知识评估。
