...
function taskQueue(status: QueueData["status"]): QueueData {
  return {
    ...
    last_error_summary: "",
    subtasks: [
      { task_id: "queue-1-task-01", sequence: 1, requirement: "新增交易", acceptance_criteria: ["可以新增"], status: status === "waiting_review" ? "waiting_review" : "pending", machine_status: status === "waiting_review" ? "success" : null, review_status: "pending", thread_id: status === "waiting_review" ? "thread-1" : null, last_error_summary: "", updated_at: "2026-07-17T08:00:00+08:00" },
      { task_id: "queue-1-task-02", sequence: 2, requirement: "交易列表", acceptance_criteria: ["可以查看"], status: "pending", machine_status: null, review_status: "pending", thread_id: null, last_error_summary: "", updated_at: "2026-07-17T08:00:00+08:00" },
    ],
...
describe("App workbench", () => {
  beforeEach(() => {
    ...
    platformApi.getProjects.mockResolvedValue([
      { project_id: "default", name: "Accounting-Software", repo_root: "/repo", is_default: true, active_identifier: null },
    ]);
  ...
  it("confirms an immutable review bound to the displayed diff", async () => {
    ...
    expect(taskApi.submitTaskReview).toHaveBeenCalledWith("task-1", {
      decision: "approved", reviewer: "Local Reviewer", comment: "Checked.", reviewed_diff_sha256: "b".repeat(64),
    });
